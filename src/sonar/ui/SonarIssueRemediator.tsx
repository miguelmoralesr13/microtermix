...
import { normalizeSonarUrl, buildAuthHeader } from '../domain';
...
export const SonarIssueRemediator: React.FC<SonarIssueRemediatorProps> = ({
    ...
    const getAuthHeader = (token: string) => {
        if (!account) return '';
        return buildAuthHeader(account.authType, token);
    };
                ...
                if (issue.rule && account?.token) {
                    ...
                    const response = await invoke('execute_http_request', {
                        request: {
                            url,
                            method: 'GET',
                            headers: { Authorization: getAuthHeader(account.token) },
                            body: null
                        }
                    }) as { is_error: boolean; error_msg?: string; status: number; body: string };
