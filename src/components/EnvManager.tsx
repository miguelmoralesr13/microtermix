        ...
        >
            ...
            >
                ...
                <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-800 shrink-0">
                    ...
                    {scriptBodies.length > 0 && (
                        <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-nexus-neon hover:bg-slate-800 rounded transition-colors"
                        >
                            <FileCode size={14} />
                        </button>
                    )}
                    ...
                    {addingEnv ? (
                        <div className="flex items-center gap-1">
                            <input
                                ...
                                className="w-36 bg-slate-950 border border-nexus-neon/50 rounded px-2 py-1 text-xs text-slate-100 focus:outline-none"
                            />
                            <button onClick={() => { addEnv(newEnvName); setNewEnvName(''); setAddingEnv(false); }}
                                className="text-nexus-success hover:bg-slate-700 p-1 rounded text-xs">✓</button>
                        ...
                        <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1 text-[11px] text-slate-500 hover:text-nexus-neon hover:bg-slate-800 rounded-lg border border-dashed border-slate-700 transition-colors"
                        >
                ...
                <div
                    ...
                    className={`mx-4 mt-2 mb-1 py-3 px-4 rounded-lg border-2 border-dashed text-center cursor-pointer transition-colors ${isDraggingFile ? 'border-nexus-neon bg-nexus-neon/10 text-nexus-neon' : 'border-slate-700 text-slate-500 hover:border-slate-600 hover:text-slate-400'}`}
                >
                    <input
                        ref={fileInputRef}
                        type="file"
                                ...
                                <tr key={key} className="border-b border-slate-800/60 hover:bg-slate-800/30 group">
                                    <td className="px-4 py-1.5">
                                        <input
                                            value={key}
                                            readOnly
                                            className="w-full bg-transparent font-mono text-nexus-neon/80 focus:outline-none"
                                        />
                                    </td>
                                    <td className="px-4 py-1.5">
                                        <input
                                    ...
                                    <td className="px-2">
                                        <button
                                            ...
                                            className="p-1 text-slate-700 hover:text-nexus-danger hover:bg-slate-700 rounded transition-colors opacity-0 group-hover:opacity-100"
                                        >
                                            <Trash2 size={12} />
                                        </button>
                                    </td>
                            ...
                            <tr className="border-b border-slate-800/60">
                                <td className="px-4 py-2">
                                    <input
                                        ...
                                        className="w-full bg-slate-800/40 border border-dashed border-slate-700 rounded px-2 py-1 font-mono text-slate-400 placeholder:text-slate-600 focus:outline-none focus:border-nexus-neon focus:text-slate-200 transition-colors"
                                    />
                                </td>
                                <td className="px-4 py-2" colSpan={2}>
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            onKeyDown={e => e.key === 'Enter' && handleAddVar()}
                                            placeholder="valor"
                                            className="flex-1 bg-slate-800/40 border border-dashed border-slate-700 rounded px-2 py-1 text-slate-400 placeholder:text-slate-600 focus:outline-none focus:border-nexus-neon focus:text-slate-200 transition-colors"
                                        />
                                        <button
                                            onClick={handleAddVar}
                                            disabled={!newKey.trim()}
                                            className="px-3 py-1 bg-nexus-neon text-nexus-darker rounded font-bold hover:bg-opacity-80 disabled:opacity-30 transition-colors"
                                        >
                                            <Plus size={13} />
                                        </button>
                                    </div>
                ...
                <div className="px-5 py-3 border-t border-slate-800 flex items-center justify-between shrink-0 bg-slate-950/40">
                    ...
                    <button
                        onClick={onClose}
                        className="px-4 py-1.5 text-xs bg-nexus-neon text-nexus-darker font-bold rounded-lg hover:bg-opacity-80 transition-colors"
                    >
                        Cerrar
                    </button>
                </div>
