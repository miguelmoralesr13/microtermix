...
Migrate the Services & Terminals section to shadcn/ui primitives, reduce boilerplate, and apply a polished visual refresh. Base: dark neon (`nexus-neon`, `nexus-accent`) with more breathing room and clearer hierarchy.
...
- Scripts: horizontal scrollable chips using shadcn `Button size="xs"`, glow on hover with `nexus-neon`
...
- shadcn `Button` variants:
  - Run → `bg-nexus-neon text-slate-900` + `Play` icon + `Badge` for selected count
...
- Active tab: `border-b-2 border-nexus-neon bg-slate-900` (neon underline instead of full border)
